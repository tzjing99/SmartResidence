...
async function main() {
  ...
  if (!existA) {
    const pkg = await p.defectReport.create({
      data: {
        condoId: condo.id, unitId: unitA.id, raisedByUserId: owner.id,
        kind: DefectReportKind.HANDOVER, title: 'Multiple defects',
    ...
    await p.defect.createMany({
      data: [
        {
          condoId: condo.id, unitId: unitA.id, raisedByUserId: owner.id, reportId: pkg.id,
          category: 'Plumbing', severity: DefectSeverity.HIGH,
          title: 'Master Bathroom - Shower: Leaking showerhead',
          description: 'Water drips continuously even when shower is fully off.',
          spaceLabel: 'Master Bathroom', location: 'Master Bathroom', status: DefectStatus.NEW,
          createdAt: new Date(now - 2 * 86400_000),
        },
        {
          condoId: condo.id, unitId: unitA.id, raisedByUserId: owner.id, reportId: pkg.id,
          category: 'Electrical', severity: DefectSeverity.MEDIUM,
        ...
        },
        {
          ...
          spaceLabel: 'Bedroom 2', location: 'Bedroom 2', status: DefectStatus.NEW,
          createdAt: new Date(now - 2 * 86400_000),
        },
      ],
    });
  ...
  if (!existB) {
    const pkg = await p.defectReport.create({
      data: {
        condoId: condo.id, unitId: unitB.id, raisedByUserId: tenant.id,
        kind: DefectReportKind.HANDOVER, title: 'Multiple defects',
    ...
    await p.defect.createMany({
      data: [
        {
          condoId: condo.id, unitId: unitB.id, raisedByUserId: tenant.id, reportId: pkg.id,
          category: 'Electrical', severity: DefectSeverity.URGENT,
          title: 'Utility Room - DB Box: Tripping breaker',
          description: 'Circuit breaker trips every evening when AC and oven run together.',
          spaceLabel: 'Utility Room', location: 'Utility Room',
          status: DefectStatus.IN_PROGRESS, assignedToUserId: admin.id,
        ...
        },
        {
          ...
          spaceLabel: 'Second Bathroom', location: 'Second Bathroom',
          status: DefectStatus.ASSIGNED, assignedToUserId: admin.id,
          acknowledgedAt: new Date(now - 4 * 86400_000),
          createdAt: new Date(now - 5 * 86400_000),
        },
        {
          condoId: condo.id, unitId: unitB.id, raisedByUserId: tenant.id, reportId: pkg.id,
          category: 'Structural', severity: DefectSeverity.MEDIUM,
          title: 'Bedroom 1 - Ceiling: Damp patch',
          description: 'Damp stain near AC unit — likely condensation leak.',
          spaceLabel: 'Bedroom 1', location: 'Bedroom 1',
          status: DefectStatus.RESOLVED, assignedToUserId: admin.id,
          acknowledgedAt: new Date(now - 4 * 86400_000),
        ...
        },
        {
          ...
          spaceLabel: 'Balcony', location: 'Balcony',
          status: DefectStatus.RESOLVED, assignedToUserId: admin.id,
          acknowledgedAt: new Date(now - 4 * 86400_000),
          resolvedAt: new Date(now - 86400_000),
          createdAt: new Date(now - 5 * 86400_000),
  ...
  if (!existC) {
    const pkg = await p.defectReport.create({
      data: {
        condoId: condo.id, unitId: unitC.id, raisedByUserId: owner.id,
        kind: DefectReportKind.HANDOVER, title: 'Multiple defects',
    ...
    await p.defect.createMany({
      data: [
        {
          condoId: condo.id, unitId: unitC.id, raisedByUserId: owner.id, reportId: pkg.id,
          category: 'Plumbing', severity: DefectSeverity.MEDIUM,
          title: 'Kitchen - Tap: Dripping',
          description: 'Kitchen tap drips when closed fully.',
          spaceLabel: 'Kitchen', location: 'Kitchen',
          status: DefectStatus.CLOSED, assignedToUserId: admin.id,
        ...
        },
        {
          ...
          spaceLabel: 'Master Bathroom', location: 'Master Bathroom',
          status: DefectStatus.CLOSED, assignedToUserId: admin.id,
          acknowledgedAt: new Date(now - 13 * 86400_000),
          resolvedAt: new Date(now - 7 * 86400_000),
          closedAt: new Date(now - 5 * 86400_000),
