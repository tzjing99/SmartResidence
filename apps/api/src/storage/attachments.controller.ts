...
export class AttachmentsController {
  ...
  async raw(
    ...
    await this.streamKey(res, attachment.key, attachment.mimeType);
  }
  ...
  private async streamKey(res: Response, key: string, contentType: string): Promise<void> {
    try {
  ...
  private async authorize(user: AuthenticatedUser, id: string) {
    const attachment = await this.prisma.attachment.findUnique({
      where: { id },
      include: { threadMessage: { include: { thread: true } } },
    });
    ...
    if (!isUploader && !canViewThread) throw new ForbiddenException();
    return attachment;
  }
