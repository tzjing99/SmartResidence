                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="text-xs sr-muted mt-0.5">
                          {row.data.category} · raised {new Date(row.data.createdAt).toLocaleDateString()}
                        </div>
                      </div>
      ...
      <Card className="transition-colors hover:border-[rgb(var(--sr-coral)/0.4)]">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            ...
            <div className="text-sm font-mono text-[rgb(var(--sr-coral))]">{defectReference(report.id)}</div>
            <div className="text-xs sr-muted mt-0.5">
              {report.itemCount} defect(s) · raised {new Date(report.createdAt).toLocaleDateString()}
            </div>
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-xs sr-muted whitespace-nowrap">{done}/{report.itemCount} fixed</span>
            </div>
          </div>
