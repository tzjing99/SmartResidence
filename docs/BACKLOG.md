...
- ✅ **F2 (partial) — Platform console basics** — `PlatformModule` + web `/admin/platform` (condo list/search, per-condo detail, setup status badges, open-condo-admin context switch) for `SUPER_ADMIN`. Feature flags store + admin toggles shipped; plan/usage metrics and support impersonation still future.
...
- ✅ **V5 — Visitor RBAC correction** — Management **read/audit only** on unit walk-ins (plus overnight-policy / overnight-approve for admins). Owners/tenants get **explicit** `create`/`read`/`delete`/`approve`/`reject` on `Visitor` (not CASL `manage`, which previously implied gate `check-in` / `create-walk-in`). Guards alone perform gate ops; service-layer `assertGateOperator` + `create()` unit ownership check harden the HTTP surface. Covered by `ability.factory.spec.ts` + `visitor.integration.spec.ts` (§2.1 cases).
...
- ✅ **M3 — Helpdesk priority-change reassignment + ML assignment scaffold (C6 seam)** — priority override now re-routes the assignee via `ThreadAssignmentService.assignOnPriorityChange` (URGENT → senior pool, etc.), not just SLA recompute. ML phase-2 seam now exists: `assignment-assist.provider.ts` + `composite-assignment-assist.provider.ts`, `ml/ml-assignment.service.ts` gated behind `ML_ASSIGNMENT_MIN_CLOSED_THREADS = 200` + an opt-in admin toggle on `/admin/settings/helpdesk`; deterministic rules remain the fallback.
- ✅ **C6 trained assignment model** — persisted Naive Bayes category model (`apps/api/ml-models/assignment-category-v1.json`) + `pnpm ml:train-assignment` offline trainer; inference returns `source: "ml"` when flag + 200-thread gate + artifact are present (rules remain fallback).
...
- ✅ **M2 — Thread auto-assignment (phase 1)** — category → pool routing, triage, recategorise reassign, repeat complainant; **F3** inbox sort, **F4** FAQ deflection, **G2** PDF export shipped in follow-on. ML (**C6**) trained category model shipped (opt-in flag + 200-thread gate + `ml:train-assignment` artifact).
...
- ✅ **Phase 2 ML** (**C6**) — trained category Naive Bayes artifact + train script + inference wired through `MlAssignmentService` / `CompositeAssignmentAssistProvider`; gated by `ML_ASSIGNMENT_MIN_CLOSED_THREADS = 200` + opt-in admin toggle; deterministic rules remain fallback. Further model quality / per-condo online retrain can iterate later.
...
- **ML assignment** ✅ — seam + 200-thread gate + opt-in flag + persisted trained category model (`ml:train-assignment`); rules remain fallback.
...
> **Status:** 🟡 Partial · **Area:** api + web · **Shipped:** `PlatformModule` + `/admin/platform` (condo list/search, detail, setup badges, provisioning, feature flag toggles)

Super admin currently has a dedicated **Platform console** at `/admin/platform` for cross-condo list/search, per-condo detail, provisioning, and **feature flag toggles** (stored under `Condo.settings.featureFlags`, audited). **Still future:** plan/usage metrics, support impersonation (audited), aggregate cross-condo health.
